...
architecture Behavorial of ppm_generate is
  ...
  -- Defaults: 1 ms channels, idle = 20ms - 7*400us - 6*1ms = 11.2 ms
  constant CH_DEFAULT   : unsigned(20 downto 0) := to_unsigned(100_000,   21);
  constant IDLE_DEFAULT : unsigned(20 downto 0) := to_unsigned(1_120_000, 21);
      ...
      if rst = '0' then
        ...
        case state is
          ...
          when GAP =>
            ppm_out <= '0';
            if timer = 0 then
              accum_v := frame_accum + GAP_TICKS;    -- single addition

              if ch_index = 6 then
                -- Compute next idle as remaining frame budget; clamp to MIN_IDLE.
                if accum_v >= FRAME_MINUS_MIN then
                  new_idle_v := to_unsigned(40_000, 21);  -- MIN_IDLE = one GAP
                else
                  new_idle_v := FRAME_TICKS - accum_v;   -- single subtraction
                end if;
                frame_accum <= (others => '0');
                ch_index    <= 0;
                timer       <= new_idle_v - 1;
          ...
          when PULSE =>
            ppm_out <= '1';
            if timer = 0 then
              frame_accum <= frame_accum + ch_widths(ch_index); 
              ch_index    <= ch_index + 1;
              timer       <= GAP_TICKS - 1;
              state       <= GAP;
            else
