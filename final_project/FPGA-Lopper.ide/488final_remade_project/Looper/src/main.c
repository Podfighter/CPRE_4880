...
int main()
{
	...
	VolumeMaster[0] = 0; // 0 = full volume
	VolumeMaster[1] = 0;
	VolumeMaster[2] = 0;
	VolumeMaster[3] = 0;

	int TrackVolumeSelect = 0;
	Xil_Out16(XPAR_GPIO_2_BASEADDR,TrackVolumeSelect+1);
	...
	while (1)
	{
		...
		for (int i = 0; i < NUM_TRACKS; i++)
		{
			...
			int Buttons = Xil_In8(XPAR_GPIO_1_BASEADDR);

			if(!(Buttons & 0x1F))
			{
				VolumeC = 1; // pedal pressed
			}

			if(Buttons & 0x10 && VolumeC == 1)//volume down
			{
				VolumeC = 0;
				if(VolumeMaster[TrackVolumeSelect] != 0)
				{
				VolumeMaster[TrackVolumeSelect]--;
				}


			}

			if(Buttons & 0x4 && VolumeC == 1)//channel left
			{
				VolumeC = 0;
				if(TrackVolumeSelect == 0)
				{
					TrackVolumeSelect = 3;
				}
				else
				...
				Xil_Out8(XPAR_GPIO_2_BASEADDR,TrackVolumeSelect+1);
			}

			if(Buttons & 0x2 && VolumeC == 1)//volume up
			{
				VolumeC = 0;
				if(VolumeMaster[TrackVolumeSelect] != 23)
				{
					VolumeMaster[TrackVolumeSelect]++;
				}
			}

			if(Buttons & 0x8 && VolumeC == 1)//channel right
			{
				VolumeC = 0;
				if(TrackVolumeSelect == 3)
				{
					TrackVolumeSelect = 0;
				}
				else
