...
void initI2S()
{
	...
	Xil_Out32(XPAR_I2S_RECEIVER_1_BASEADDR + 0x30, 0x01);
	Xil_Out32(XPAR_I2S_RECEIVER_1_BASEADDR + 0x0C, 0x01);
	Xil_Out32(XPAR_I2S_RECEIVER_1_BASEADDR + 0x08, 0x01);

	Xil_Out32(XPAR_I2S_RECEIVER_2_BASEADDR + 0x30, 0x01);
	Xil_Out32(XPAR_I2S_RECEIVER_2_BASEADDR + 0x0C, 0x01);
	Xil_Out32(XPAR_I2S_RECEIVER_2_BASEADDR + 0x08, 0x01);
...
void initDma()
{
	...
	XAxiDma_Config *cfg3 = XAxiDma_LookupConfig(XPAR_AXIDMA_2_DEVICE_ID);
	XAxiDma_CfgInitialize(&Dmadual, cfg3);
	...
	XAxiDma_IntrDisable(&Dmadual, XAXIDMA_IRQ_ALL_MASK, XAXIDMA_DEVICE_TO_DMA);
	XAxiDma_IntrDisable(&Dmadual, XAXIDMA_IRQ_ALL_MASK, XAXIDMA_DMA_TO_DEVICE);

	// let's use the dma rec interrupts... need these.

	XAxiDma_IntrEnable(&Dmarec, XAXIDMA_IRQ_ALL_MASK, XAXIDMA_DEVICE_TO_DMA);
}

// void recHandler(void* callback){
//
//
//	u32 stattie = XAxiDma_IntrGetIrq(&Dmarec,XAXIDMA_DEVICE_TO_DMA);
//	XAxiDma_IntrAckIrq(&Dmarec,stattie,XAXIDMA_DEVICE_TO_DMA);
//	//need to call some shit up!
//	count++;
//
//	//call the next record stage
//	if(count < 1000){
//	record(count);
//
//
