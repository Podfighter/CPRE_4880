							  ...
							  size_t count, loff_t *ppos)
{
	...
	thisblows = kzalloc(8, GFP_KERNEL);

	thisblows[0] = 0x02;
	get_user(thisblows[1], &user_buffer[0]);
	...
	retval = usb_control_msg(
		dev->udev,
		// we need to grab the actual device
		usb_sndctrlpipe(dev->udev, 0),
		// establish the pipe to send the data to the launcher through.

		0x09,
		0x21,
		0,
		0,
		// pulled these from that one dude's blog, should work.

		thisblows,
		// everything else just does buf, so IN THEORY this would be kosher. in theory.
		8,
		// ihould this be 8...???
		// this isn't null-terminated, i dont think. I think setting it to 8 would be safest. Still, I'm going to try that later.

		10 * HZ
		// run FOREVER until it gets thru. i dont know the actual amount it should be... uh.. yeah, could be an issue. will tweak if it breaks.
	);
	...
	/* Free the unused bulk URB and its DMA buffer, restore semaphore */
