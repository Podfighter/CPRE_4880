...
#include "../../src2/NESCore/M6502_rw.h"
#include "../../src2/NESCore/NESCore.h"
#include "../../src2/NESCore/Tables.h"

#define BRANCH_CYCLE 48
#define PAGE_CROSS_CYCLE 48
#define MASTER_CYCLE_SHIFT_CNT 4
...
#define MC_Ax_P(Rg)	M_LDWORD(Rg);Rg.W+=R->X; \
					if((Rg.W & 0xFF) < R->X) \
					S.RunCycles-=PAGE_CROSS_CYCLE
#define MC_Ay(Rg)	M_LDWORD(Rg);Rg.W+=R->Y
#define MC_Ay_P(Rg)	MC_Ay(Rg);if((Rg.W & 0xFF) < R->Y)\
			S.RunCycles-=PAGE_CROSS_CYCLE
...
#define MC_Iy(Rg)       K.W=Op6502(R->PC.W++); \
			Rg.B.l=Op6502(K.W++);Rg.B.h=Op6502(K.W); \
			Rg.W+=R->Y
#define MC_Iy_P(Rg)  MC_Iy(Rg);if((Rg.W & 0xFF) < R->Y)\
			S.RunCycles-=PAGE_CROSS_CYCLE
...
#define	MR_Ax(Rg)	MC_Ax_P(J);Rg=Rd6502(J.W)
#define MR_Ay(Rg)	MC_Ay_P(J);Rg=Rd6502(J.W)
#define MR_Ix(Rg)	MC_Ix(J);Rg=Rd6502(J.W)
#define MR_Iy(Rg)	MC_Iy_P(J);Rg=Rd6502(J.W)
...
#define M_LDWORD(Rg)	Rg.B.l=Op6502(R->PC.W++); \
						Rg.B.h=Op6502(R->PC.W++)

#define M_PUSH(Rg)	Wr6502(0x0100|R->S,Rg);R->S--
#define M_POP(Rg)	R->S++;Rg=Op6502(0x0100|R->S)
/** Yes, there is an edge case where page cross cycles are not correct */
/** I didn't feel like rewriting to handle it */
#define M_JR		S.RunCycles-=BRANCH_CYCLE;T=R->PC.W; \
					R->PC.W+=(offset)Op6502(R->PC.W)+1; \
					if ((R->PC.W & 0xFF00) != (T & 0xFF00)) \
					S.RunCycles-=PAGE_CROSS_CYCLE;R->ICount--
...
int Exec6502(M6502 *R,int RunCycles)
{
  ...
  while(S.RunCycles>0)
  {
    ...
    if(R->Trace)
      if(!Debug6502(R)) return(S.RunCycles);
#endif

    I=Op6502(R->PC.W++);
	/* effectively multiplies by 16 (note cycles for instructions are already
	 * multiplied by 3. We get the multiply by 48 with a simple shift operation
	 * rather than converting all of the cycle counts to words... */
    S.RunCycles-=((int)Cycles[I] << MASTER_CYCLE_SHIFT_CNT);
    switch(I)
    {
#include "../../src2/NESCore/Codes.h"
    }
  }
