...
/*****************************************************************************
 * NESCore.c - Top-level functionality of the NESCore emulator. 
 *
 *
 * NOTES:
 * 10/24/13 by JAZ::Design created.
 *****************************************************************************/
...
int NESCore_LoadROM(const char *fileName) {
	...
	if (status != XST_SUCCESS) {
		xil_printf("NESCore_LoadROM(): Error reading romfile %s\r\n", fileName);
		return E_INVALID_FILE;
	}

	/* Read ROM Header */
	//  fread(&S.NesHeader, sizeof(S.NesHeader), 1, fp);
	tmpPtr = (uint8_t *)&S.NesHeader;
	xilsd_fread(tmpPtr, 1, sizeof(S.NesHeader), fp);

	if (memcmp(S.NesHeader.ID, "NES\x1a", 4 ) != 0 ) {
		/* Not a valid .NES ROM */
		xil_printf("NESCore_LoadROM(): %s is not a valid .nes ROM\r\n", fileName);
		xilsd_fclose(fp);
	...
	if (S.NesHeader.Info1 & 4) {
		...
		if (bootstate.debug_level >= 2) {
			xil_printf("NESCore_LoadROM(): Warning - extra header info\r\n");
		}
	}
	...
	tmpPtr = (uint8_t *)W.ROM;
	xilsd_fread(tmpPtr, 1, 0x4000*S.NesHeader.ROMSize, fp);
	if (bootstate.debug_level >= 2) {
		xil_printf("NESCore_LoadROM(): ROMSize is %d\r\n", S.NesHeader.ROMSize);
		xil_printf("NESCore_LoadROM(): VROMSize is %d\r\n", S.NesHeader.VROMSize);
	}
	...
	if (S.NesHeader.VROMSize > 0) {
		...
		tmpPtr = (uint8_t *)W.VROM;
		xilsd_fread(tmpPtr, 1, 0x2000*S.NesHeader.VROMSize, fp);
...
void NESCore_Callback_Wait() {
  ...
  int32_t tv, usec_this_frame;
  int32_t delta;


  // Grab the initial timer
  if (ptv == 0) {
    ptv = (int32_t)XTmrCtr_GetValue(&TmrInstance, 0);
    return;
  }

  tv = (int32_t)XTmrCtr_GetValue(&TmrInstance, 0);
  usec_this_frame = (int32_t)11.42857/1000*(tv-ptv);

  delta = usec_this_frame - usec_this_frame;
  if (delta > 0) {
    usleep(delta);
...
int NESCore_SaveState(const char *fn) {
	...
	if (bootstate.debug_level >= 1)
		xil_printf("NESCore_SaveState(): saving state\r\n");
 

	status = xilsd_fopen(fp, fn);
	if (status != XST_SUCCESS)
		return E_FILE;

	//xilsd_fwrite(&S, sizeof(S), 1, fp);
	//sysace_fwrite(&W, sizeof(W), 1, fp);

	if (M.state && M.size)
		//xilsd_fwrite(M.state, M.size, 1, fp);
...
int NESCore_LoadState(const char *fn) {
	...
	if (bootstate.debug_level >= 1)
		xil_printf("NESCore_LoadState(): loading state\r\n");

	status = xilsd_fopen(fp, fn);
	if (status != XST_SUCCESS)
		return -1;

	xilsd_fread(&S, sizeof(S), 1, fp);
	xilsd_fread(&I, sizeof(W), 1, fp);

	if (M.state && M.size)
		xilsd_fread(M.state, M.size, 1, fp);
...
void NESCore_Init() {
  ...
  if (bootstate.debug_level >= 1)
    xil_printf("NESCore_Init(): initializing emulator core\r\n");
...
int NESCore_Reset()
{
  ...
  if (bootstate.debug_level >= 1)
    xil_printf("NESCore_Reset(): resetting the emulator\r\n");
  ...
  if (bootstate.debug_level >= 2) {
    xil_printf("ROM Type: %s\r\n", (S.PAL) ? "PAL" : "NTSC");
    xil_printf("Mirroring: %d\r\n", S.ROM_Mirroring);
    xil_printf("SRAM: %s\r\n", (S.ROM_SRAM) ? "YES" : "NO");
    xil_printf("4-Screen Mirroring: %s\r\n", (S.ROM_FourScr) ? "YES" : "NO");
  }
  ...
  S.PAD1_Latch = S.PAD2_Latch = S.PAD1_Bit = S.PAD2_Bit =0;


  // Initialize the Picture Processing Unit
  if (bootstate.debug_level >= 4)
    xil_printf("NESCore_Reset(): Initializing PPU\r\n");

  NESCore_Init_PPU(); 


  // Initialize the Pseudo-Audio Processing Unit
  if (bootstate.debug_level >= 4)
    xil_printf("NESCore_Reset(): Initializing pAPU\r\n"); 
  ...
  if (bootstate.debug_level >= 4)
    xil_printf("NESCore_Reset(): Initializing mapper %d [Idx %d]\r\n",
	       S.MapperNo, S.MapperIdx); 

  MapperTable[ S.MapperIdx ].pMapperInit();


  // Initialize the CPU
  if (bootstate.debug_level >= 4)
    xil_printf("NESCore_Reset(): Initializing 6502\r\n");   
  ...
  for (;;) {    
      ...
      if ( S.PPU_Scanline == S.SpriteJustHit && 
           PPU_ScanTable[S.PPU_Scanline] == SCAN_ON_SCREEN )
        {
          ...
          if ((S.PPU_R1 & R1_SHOW_SP)  && (S.PPU_R1 & R1_SHOW_SCR))
            S.PPU_R2 |= R2_HIT_SP;
        }
...
int NESCore_HSync() {
  ...
  if (S.FrameCnt == 0 && PPU_ScanTable[ S.PPU_Scanline ] == SCAN_ON_SCREEN)
  ...
  switch (S.PPU_Scanline)
    ...
    case SCAN_TOP_OFF_SCREEN:
      ...
      if (S.NesHeader.VROMSize == 0 && S.FrameCnt == 0)
...
void NESCore_DrawScanline()
{
  ...
  if (S.FrameCnt == 0 && (S.PPU_R1 & R1_SHOW_SP))
    {
