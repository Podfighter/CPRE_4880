...
#include <stdli#include "../src2/nes_bootloader.h"
#include "../src2/NESCore/NESCore.h"
rc2/NESCore/NESCore.h"
...
void NESCore_Callback_OutputFrame(word *WorkFrame) {
	...
	uint16_t tpixel;
	uint16_t swapper = 0x0000;
/*
 * NES is outputting at 256x240...
 * Screen is at 640x480
 * 256x2 = 512, need to chop off 640 - 512 = 128 / 2 = 64 bits off of each side
 * pixels need to be doublecounted, or drawn as 2x2 proportionally to the frame
 *
 *
 */
int r,k = 0;

//TODO: put this in a separate function so it's not being repeatedly redrawn to save time and
//to reduce tearing. yay! it'd be saving 128 * 480 writes per frame!

	for(r = 0;r < 480; r++){
	    	for(k = 0;k < 64;k++){
	    	ptr[r*640 + k] = 0x0000;
	    	ptr[r*640 + (640 - 64) + k] = 0x0000;
	    	//set black with borders that are 64 pix wide
	    }
	    }
	...
	for (i = 0 ; i < NES_DISP_HEIGHT; i++) {
		...
		for (j = 0; j < NES_DISP_WIDTH; j++) {
			...
			//first, let's fill the borders of the screen!

			// Grab a temporary pixel using the color palette lookup table.
			tpixel = NesPalette3[WorkFrame[NES_DISP_WIDTH*i+j]];
/*			swapper |= tpixel & 0x0F0;
			swapper |= ((tpixel & 0xF00) >> 8);
			swapper |= ((tpixel & 0x00F) << 8);*/

			/*
			ptr[i*2][64 + j*2] = tpixel;
			ptr[i*2 + 1][64 + j*2] = tpixel;
			ptr[i][65 + j*2] = tpixel;
			ptr[i][65 + j*2] = tpixel;
			*/
			//need to redo this but with a 1D array... fuck my life
			//oh nevermind it was easy lol





			ptr[j*2 + 640 * i * 2 + 64] = tpixel << 4;

			ptr[j*2 + 640 * i * 2 + 65] = tpixel << 4;

			ptr[j*2 + 640 * i * 2 + 64 + 640] = tpixel << 4;

			ptr[j*2 + 640 * i * 2 + 65 + 640] = tpixel << 4;

//			ptr[j*2 + 640 * i + 64 + 640*240] = tpixel << 4;
//			ptr[j*2 + 640 * i + 65 + 240*640] = tpixel << 4;

//			ptr[j + 640 * i + 65] = tpixel << 4;
//			ptr[j*2 + 640 * 2*i + 64] = tpixel << 4;
//			ptr[j*2 + 640 * 2*i + 65] = tpixel << 4;


			//swapper = 0x0000;
//			ptr[i*2 + 65] = tpixel;
//			ptr[i + 64 + 640 * j] = tpixel;
//			ptr[i + 65 + 640 * j] = tpixel;

			//this draws pixels in 4x4 squares, with offsets of 64.
			//the outside of the buffer should remain 0... presumably. (the outer edge that's 64 wide.)
