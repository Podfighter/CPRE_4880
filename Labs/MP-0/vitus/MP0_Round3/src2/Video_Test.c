...
#include "xvtc.h"
#include "xaxivdma.h"
#include "../src2/plat#include "../src2/platform.h"
form.h"
    ...
    for (i = 0; i < 480; i++) {
      for (j = 0; j < 640; j++) {
        ...
        if (j < 213) {
        	...
        	test_image[i][j] = 0x00f; // red pixels
        }
        else if(j < 426 ) {
          test_image[i][j] = 0x0f0; // green pixels
        }
        else {
        	test_image[i][j] = 0xf00; // blue pixels
        }
...
*/
    ...
    for(i=0; i< 480;i++)
    {
    	for(j=0; j<640; j++)
    	{
    		...
    		if(j==0||j==639)
    			...
    			test_image[i][j] = 0x0C13;
    		else
    			test_image[i][j] = 0x0fB4;
    	}
    }
    ...
    XAxiVdma_WriteReg(XPAR_AXI_VDMA_0_BASEADDR, XAXIVDMA_HI_FRMBUF_OFFSET, 0x1);  // Read Channel: VDMA MM2S Reg_Index
