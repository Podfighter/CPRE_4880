...
void Cannon_object_detect(void)
{
  ...
  Xuint16 buf2 = pS2MM_Mem[12*1920 + 5] & mask;
...
//			  even_odd++;
			  ...
			  if (target.found == 1 && col == target.x){
				  pMM2S_Mem[row*1920 + col] = mask;
			  }
			  else if(target.found == 1 && row == target.y){
				  pMM2S_Mem[row*1920 + col] = mask;
			  }
