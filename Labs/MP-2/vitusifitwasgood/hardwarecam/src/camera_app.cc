...
#include "./cannon_object_detect.h"
...
void camera_interface(volatile Xuint16* pMM2S_Mem, volatile Xuint16* pS2MM_Mem){
	...
	Xuint16 snapshot[32][1080][1920] = {0};
	Xuint16 video[50][1080][1920] = {0};
	Xuint16 buf[1080][1920] = {0};

	//let's do video. Shouldn't be that hard? let's do 10 fps, also (allocate it on the heap...?)
	signed char index = 0;
	signed char zoom = 0;
	//0 is lowest, 3 is highest
	//x1 = 0, x2 = 1, x4 = 2, x 8 = 3
	...
	vdma_MM2S_DMACR = XAxiVdma_ReadReg(XPAR_AXIVDMA_0_BASEADDR, XAXIVDMA_TX_OFFSET+XAXIVDMA_CR_OFFSET);
		...
		b_read = Xil_In8(XPAR_AXI_GPIO_0_BASEADDR);
		s_read = Xil_In8(XPAR_AXI_GPIO_1_BASEADDR);

		//what zoom do we want to use?
		if(b_read & 0x10){
			zoom++;
			zoom = (zoom > 2) ? 0 : zoom;
			sleep(0.1);
		...
		case 0:
			for (int row = 0; row < 1080; row++) {
						 		  for(int col = 0;col < 1920;col++){
						 			  ...
						 			  //grab vals
						 			  buf[row][col] = pS2MM_Mem[row*1920 + col];
						 		  }
						  }
		...
		case 1:
			...
			for (int row = 0; row < 540; row++) {
						 		  for(int col = 0;col < 960;col++){
						 			  ...
						 			  //grab vals
						 			  buf[row*2][col*2] = pS2MM_Mem[row*1920 + 1920*270 + col + 480];
						 			 buf[row*2][col*2 + 1] = pS2MM_Mem[row*1920 + 1920*270 + col + 480];
						 			buf[row*2 + 1][col*2] = pS2MM_Mem[row*1920 + 1920*270 + col + 480];
						 			buf[row*2 + 1][col*2 + 1] = pS2MM_Mem[row*1920 + 270*1920 + col + 480];
						 		  }
						  }
			...
			for (int row = 0; row < 270; row++) {
									 		  for(int col = 0;col < 480;col++){
									 			  ...
									 			  //grab vals
							                        ...
							                        buf[row*4 + 2][col*4 + 2] = pS2MM_Mem[row*1920 + 1920*405 + col + 720];
																 			buf[row*4 + 2][col*4 + 3] = pS2MM_Mem[row*1920 + 1920*405 + col + 720];
																 			buf[row*4 + 3][col*4 + 2] = pS2MM_Mem[row*1920 + 1920*405 + col + 720];
																 			buf[row*4 + 3][col*4 + 3] = pS2MM_Mem[row*1920 + 405*1920 + col + 720];

							                        buf[row*4][col*4 + 2] = pS2MM_Mem[row*1920 + 1920*405 + col + 720];
																 			buf[row*4][col*4 + 3] = pS2MM_Mem[row*1920 + 1920*405 + col + 720];
																 			buf[row*4 + 1][col*4 + 2] = pS2MM_Mem[row*1920 + 1920*405 + col + 720];
																 			buf[row*4 + 1][col*4 + 3] = pS2MM_Mem[row*1920 + 405*1920 + col + 720];

							                        buf[row*4 + 2][col*4] = pS2MM_Mem[row*1920 + 1920*405 + col + 720];
																 			buf[row*4 + 3][col*4] = pS2MM_Mem[row*1920 + 1920*405 + col + 720];
																 			buf[row*4 + 2][col*4 + 1] = pS2MM_Mem[row*1920 + 1920*405 + col + 720];
																 			buf[row*4 + 3][col*4 + 1] = pS2MM_Mem[row*1920 + 405*1920 + col + 720];
		...
		vdma_MM2S_DMACR = XAxiVdma_ReadReg(XPAR_AXIVDMA_0_BASEADDR, XAXIVDMA_TX_OFFSET+XAXIVDMA_CR_OFFSET);
		  ...
		  for (int row = 0; row < 1080; row++) {
		 		  for(int col = 0;col < 1920;col++){
		 			  ...
		 			  //grab vals
		 			 pMM2S_Mem[row*1920 + col] = buf[row][col];
		 		  }
		  }
		...
		if(b_read & 0x01){
			...
			//index can only range from 0 to 31, if we're past 31, just start overwriting old images.
			...
			//record it
			  for (int row = 0; row < 1080; row++) {
			 		  for(int col = 0;col < 1920;col++){
			 			  ...
			 			  //grab vals
			 			  snapshot[index][row][col] = buf[row][col];
			 		  }
			  }


			//wait for 2 sec, then unpause vdma!
			sleep(2);
		...
		//This nested while is where i'll do the playback thing:
		...
		while((s_read & 0x1)){
			...
			b_read = Xil_In8(XPAR_AXI_GPIO_0_BASEADDR);
					...
					pMM2S_Mem[row*1920 + col] = snapshot[index][row][col];
		...
		for (int frame = 0; frame < 50;frame++){
			...
			for (int row = 0; row < 1080; row++) {
				for(int col = 0;col < 1920;col++){
					...
					video[frame][row][col] = pS2MM_Mem[row*1920 + col];
				}
		...
		if(s_read & 0x4){
		//do nothing until the switch ISNT flipped then immediately exit loop and die
			while(s_read & 0x4){
				s_read = Xil_In8(XPAR_AXI_GPIO_1_BASEADDR);
			}
			...
			for (int frame = 0; frame < 50;frame++){
				...
				for (int row = 0; row < 1080; row++) {
					for(int col = 0;col < 1920;col++){
						...
						pMM2S_Mem[row*1920 + col] = video[frame][row][col];
					}
...
//  }
  ...
  camera_interface( pMM2S_Mem, pS2MM_Mem);
...
int enable_color_pipeline(void) {
   ...
   Xil_Out32((XPAR_V_PROC_SS_1_BASEADDR + 0x10),(u32)1920);
   Xil_Out32((XPAR_V_PROC_SS_1_BASEADDR + 0x18),(u32)1080);

   Xil_Out32((XPAR_V_PROC_SS_1_BASEADDR + 0x20),(u32)1);

   Xil_Out32((XPAR_V_PROC_SS_1_BASEADDR + 0x28),(u32)2);
