...
//  enable_color_pipeline();
  ...
  volatile unsigned int *TPG_BOXSIZE      = (unsigned int *)(XPAR_V_TPG_0_S_AXI_CTRL_BASEADDR + 0x78); // Box Size
  volatile unsigned int *TPG_R      = (unsigned int *)(XPAR_V_TPG_0_S_AXI_CTRL_BASEADDR + 0x80); // Red (Box Color)
  volatile unsigned int *TPG_G      = (unsigned int *)(XPAR_V_TPG_0_S_AXI_CTRL_BASEADDR + 0x88); // Green (Box Color)
  volatile unsigned int *TPG_B      = (unsigned int *)(XPAR_V_TPG_0_S_AXI_CTRL_BASEADDR + 0x90); // Blue (Box Color)
  ...
  TPG_BGP[0]    = 0x09;  // Background Pattern           // MODIFICATION: Changed from 0x0A to 0x09 Color bars
  TPG_FGP[0]    = 0x01;  // Foreground Pattern           // MODIFICATION: Changed from 0x00 to 0x01 Moving box
  TPG_MS[0]     = 0x10;  // Motion Speed                 // MODIFICATION: Changed from 0x04 to 0x10
  TPG_CF[0]     = 0x02;  // TPG Color Format
  TPG_CR[0]     = 0x81;  // TPG Control

  TPG_BOXSIZE[0]	= 0xAA;	//Box Size
  TPG_R[0]	= 0x99;	//Red Bits
  TPG_G[0]	= 0xFF; //Green Bits
  TPG_B[0]	= 0x33; //Blue Bits
